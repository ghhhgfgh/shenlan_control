...
void PathPlanningNode::plannerTimerLoop(const ros::TimerEvent &) {
  if (!firstRecord_ && !isReachGoal_) {  //有定位数据开始规划
    ...
    s0_ = ego_s;
    double theta_r = csp_obj_ -> calc_yaw (s0_);
    double theta_x = vehicleState_.heading;
    double kappa_r = csp_obj_ -> calc_curvature(s0_);

    if (std::abs(ego_speed) > 1e-3) {
      c_speed_ = ego_speed * std::cos(theta_x - theta_r) / (1.0 - kappa_r * ego_l);}
      //c_speed_ = ego_speed;}
    c_d_ = ego_l;
    //c_d_d_ = (1.0 - kappa_r * ego_l) * std::tan(theta_x - theta_r);
